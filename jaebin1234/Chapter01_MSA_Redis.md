...
2. SAVE, BGSAVE 란? 서비스 중 SAVE, BGSAVE 사용하면 안 되는 이유는?
    - SAVE : 현재 Redis의 데이터를 동기적으로 디스크에 저장
    - BGSAVE : Redis의 데이터를 비동기적으로 백그라운드에서 저장.
    - SAVE는 동기 방식으로 Redis를 블로킹하므로 서비스 중 사용하면 안 됩니다.
    - BGSAVE는 비동기지만 fork 시 메모리 급증으로 장애 가능성이 있어 주의해야 합니다.
    - 따라서, 실시간 처리 중에는 요청 지연, 스왑 발생, 심한 경우 Redis 다운까지 발생할 수 있어 서비스 중 사용은 지양해야 합니다.
    - 실무에서는 복제 노드나 AOF 방식 등을 활용한 백업 전략이 안전합니다.
---
